...
int initialize::read_and_write_file(int mode, int length,
  ...
  if (mode != DELETE_DUPLICATES) {
    ...
    if (file_read.is_open() && file_write.is_open()) {
      ...
      std::cout << "File edited, "
                << "execution took: " << elapsed.count() << " seconds"
                << ", exiting."
                << "\n";
      ...
      std::cout << "Error opening files for reading or writing, there is a "
                   "problem with your file paths."
                << std::endl;
      return 1;
    }
  } else {
...
void initialize::init_mode_func(const int mode, std::string_view line,
                                const int length, std::string_view append,
                                std::string &shell) {
  switch (mode) {
